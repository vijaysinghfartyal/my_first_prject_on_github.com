...
public class RegisterServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        out.print(".button { padding: 10px 15px; font-size: 16px; color: white; background-color: #007bff; border: none; border-radius: 5px; cursor: pointer; text-decoration: none; }");
        ...
        rb.setUsername(request.getParameter("username").trim());
        rb.setPassword(request.getParameter("password").trim());
        ...
        if(rb.register()) {
            ...
            out.print("<p class='message'>Your account has been created successfully.</p>");
            out.print("<a href='Login.html' class='button'>Go to Login</a>");
        } else {
            out.print("<h1>Input Data Error</h1>");
            out.print("<p class='message'>There was an error during registration. Please try again.</p>");
            out.print("<a href='register.html' class='button'>Retry</a>");
        }
